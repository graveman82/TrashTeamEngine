...
#include "MemMng/Zv3DMemDefs.h"
#include "MemMng/Zv3DMemUtils.h"
#include "Debug/Zv3DAssert.h"
#include "Utils/Zv3DSmallCommonUtils.h"
...
void* ZvdMalloc(ZvdSize nBytes, const char* initiatorName, const char* comment)
{
    ...
    {   // not memory or something go wrong
        ...
        ZvdSafeStrCpyR(errorTextualInfo, comment, comment + ZvdMin(nCommentLen + 1, knErrorTextualInfoSize));
...
{
    ZVD_ASSERT_HIGH(dst, ZVD_ASSERT_TEXT("dst is null"));
    ZVD_ASSERT_HIGH(src, ZVD_ASSERT_TEXT("src is null"));
    if (dstSize)
    {
        ZVD_ASSERT_HIGH(nBytes <= *dstSize, ZVD_ASSERT_TEXT("trying dest buffer to overflow!"));

        return dst;
    }
    ZVD_ASSERT_HIGH(ZvdAbsPtrDist(dst, src) >= nBytes, ZVD_ASSERT_TEXT("overlapped"));
